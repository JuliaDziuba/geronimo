...
class Activity < ActiveRecord::Base
  ...
  scope :currentConsignmentsAtVenueBetweenDates, lambda { |venue, date_start, date_end| where('activities.venue_id = ? AND activities.date_start >= ? AND activities.date_start <= ? AND activitycategory_id = ? AND (activities.date_end isNull OR activities.date_end >= ?)', venue, date_start, date_end, Activitycategory::CONSIGNMENT[:id], date_start) }
  scope :consignmentsAtVenueBetweenDates, lambda { |venue, date_start, date_end| where('activities.venue_id = ? AND activities.date_start >= ? AND activities.date_start <= ? AND activitycategory_id = ?', venue, date_start, date_end, Activitycategory::CONSIGNMENT[:id]) }
